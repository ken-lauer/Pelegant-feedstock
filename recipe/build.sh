...
echo "SDKROOT was $SDKROOT but we're unsetting it..."
unset SDKROOT
...
make Pelegant \
  ...
  USER_MPI_FLAGS="-DUSE_MPI=1 -DSDDS_MPI_IO=1 -I$PREFIX/include" \
   || { echo waiting; sleep 1000000000; exit; }
popd
...
for fn in "${SRC_DIR}/oag/apps/bin/${EPICS_HOST_ARCH}/"* "${SRC_DIR}/epics/extensions/bin/${EPICS_HOST_ARCH}/"*; do
  "${INSTALL_NAME_TOOL}" -add_rpath "$PREFIX/lib" "$fn"
done
